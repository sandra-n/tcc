...
def load_lstm_cnn_emb_model():
    ...
    try:
        ...
        model.load_state_dict(torch.load(path + "/model/lstm_cnn_embedded/model_lstm_cnn_emb", map_location=torch.device('cpu')))
        model.eval()
        return model
...
def tweets_tok(tweet):
    cntvct = load(path + '/model/lstm_cnn_embedded/cntvct.joblib')
    svd = load(path + '/model/lstm_cnn_embedded/svd.joblib')
